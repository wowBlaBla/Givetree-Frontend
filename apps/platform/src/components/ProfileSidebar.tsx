  ...
  return (
    <div className="hidden sm:flex flex-col w-75 py-8 bg-white border-r border-base-content border-opacity-25 dark:bg-mid-dark min-h-screen">
      ...
      <div className="flex flex-col justify-between flex-1 mt-6">
        <nav>
          {navs.map((item, idx) => (
            <Link href={"/profile/" + params?.role + "/" + item.category} key={idx}>
              <a
                className={`flex items-center p-4 transition-colors duration-300 transform dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700 dark:hover:text-gray-200 hover:text-gray-700 ${
                  item.category == params?.category
                    ? "bg-menu-orange text-white"
                    : " text-gray-600 "
                }`}
                href=""
              >
