...
export const SignupWithEmail:FC = () => {
    ...
    const registerByEmail = async() => {
        try {
            ...
            setLoading(true);
            await axios.post(`${process.env.BACKEND}/api/auth/register-email`, payload).then(res => {
            ...
            <div className="flex flex-col gap-3 py-10 border rounded-lg bg-gray-600 border-base-content border-opacity-50 px-12 w-full">
                ...
                <div className="form-layout flex flex-col gap-2">
                    <div className="input-form-group flex flex-col items-start">
                        ...
                        <input
                            type="text"
                            className={`outline-none focus:border-sky-400 mt-1 block w-full sm:text-sm text-white border rounded-md p-3 bg-transparent ${errors?.username ? "border-red-500" : "border-gray-300 border-opacity-25 "}`}
                    ...
                    <div className="input-form-group flex flex-col items-start">
                        ...
                        <input
                            type="password"
                            className={`outline-none focus:border-sky-400 mt-1 block w-full sm:text-sm text-white border rounded-md p-3 bg-transparent ${(errors?.password || password != securePassword) ? "border-red-500" : "border-gray-300 border-opacity-25 "}`}
                            placeholder="Enter a secure password"
                            value={password}
                            onChange={(e) => setPassword(e.target.value)}
                            disabled={isLoading}
                        />
                        <span className="text-red-500 text-xs mt-1">{errors.password ? errors.password : (password != securePassword ? "Password is incorrect" : "")}</span>
                    </div>
                    <div className="input-form-group flex flex-col items-start">
